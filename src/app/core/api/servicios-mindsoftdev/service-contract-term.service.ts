...
export class ServiceContractTermService {
  ...
  getDeletedServiceContractTerms(): Observable<IServiceContractTermsApiResponse> {
    const customHeaders = this.customHeadersService.addAppJson().getHeaders();
    return this.http.get<IServiceContractTermsApiResponse>(`${this.apiServiceTerms}/deleted`, {
      headers: customHeaders,
      withCredentials: true,
    });
  }
  ...
  restoreServiceContractTerm(id: number): Observable<HttpResponse<any>> {
    const customHeaders = this.customHeadersService.addAppJson().addXsrfToken().getHeaders();
    return this.http.put<any>(`${this.apiServiceTerms}/${id}/restore`, null, {
      headers: customHeaders,
      withCredentials: true,
      observe: 'response'
    });
